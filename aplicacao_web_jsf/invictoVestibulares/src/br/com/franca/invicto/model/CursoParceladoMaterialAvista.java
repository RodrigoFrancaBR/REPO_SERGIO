...
public class CursoParceladoMaterialAvista implements CondicaoDoContrato {
	...
	public List<Parcela> calculaParcelas(Contrato contrato) {
		...
		for (int i = 2; i <= contrato.getQtdParcelasCurso(); i++) {
			...
			parcela.setValorDaParcelaDoCurso(cursoComDesconto.divide(BigDecimal.valueOf(contrato.getQtdParcelasCurso()),
			...
			if (i == contrato.getQtdParcelasCurso()) {
				...
				parcela.setValorDaParcelaDoCurso(
						parcela.getValorDaParcelaDoCurso()
			...
			/*
			 * parcela.setValorTotalDaParcela(
			 * parcela.getValorDaParcelaDoCurso().add(parcela.getValorResidualDaParcelaCurso
			 * ()));
			 */
