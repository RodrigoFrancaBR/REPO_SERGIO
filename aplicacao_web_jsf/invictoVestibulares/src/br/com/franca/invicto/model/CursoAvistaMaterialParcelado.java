...
public class CursoAvistaMaterialParcelado implements CondicaoDoContrato {
	...
	public List <Parcela> calculaParcelas(Contrato contrato) {
		...
		parcela.setValorDaParcelaDoCurso(contrato.getValorCurso().subtract(
				desconto));
		...
		for (int i = 1; i <= contrato.getQtdParcelasMaterial(); i++) {			
			...
			parcela.setValorDaParcelaDoMaterial(contrato
					.getValorMaterial()
			...
			if (i == contrato.getQtdParcelasMaterial()) {
				...
				parcela.setValorResidualDaParcelaMaterial(contrato.getValorMaterial().subtract(parcela.getValorDaParcelaDoMaterial()
			...
			parcela.setValorTotalDaParcela(parcela
					.getValorResidualDaParcelaMaterial().add(
